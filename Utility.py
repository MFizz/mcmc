...
class covarianceCalculator(object):
    ...
    def getSampleCovariance(self, samples):
        ...
        self.mean = self.mean + 1./(samples.shape[0]) * (lastSample - self.mean)
        self.covariance = self.covariance + 1./(samples.shape[0]) * ( np.dot((lastSample - self.mean) , (lastSample - self.mean).transpose()) - self.covariance)
      #  print np.cov(np.matrix(samples).transpose())
        #return np.cov(np.matrix(samples).transpose())
...
def getSuboptimality(covarianceMatrix, realCovarianceMatrix):
    ...
    sum = 0
    mat = np.dot(np.matrix(covarianceMatrix),np.matrix(realCovarianceMatrix)**(-1))
    print "a"
    mat = np.nan_to_num(mat)
    print "b"
    eigenvalues, evectors = np.linalg.eig(sp.linalg.sqrtm(mat))
    bs = []
